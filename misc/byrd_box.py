...
def emit_code(label, body, goto): c_source.append("%-4s%-20s%-50s%s" % ('', label, body, goto))
#------------------------------------------------------------------------------
def program_head():
...
def genc(t):
    ...
    match t[0]:
        case 'ICON':
            ...
            dump_tree(t)
            program_head()
            emit_code(f'',              f'', f'goto {L}_start;')
            E = genc(t[1])
            emit_code(f'{L}_start:',    f'', f'goto {E}_start;')
            emit_code(f'{L}_resume:',   f'', f'return; ')
            emit_code(f'{E}_fail:',     f'write_str(out, cszFailure);', f'return;')
            emit_code(f'{E}_succeed:',  f'write_str(out, cszSuccess);', f'goto {E}_resume;')
            program_tail()
        case 'EVERY': L = genc(t[1])
        case 'I':
            L = f'x{counter}_{t[1]}'
            emit_decl(f'int',           f'{L}_value;')
            emit_code(f'{L}_start:',    f'{L}_value = {t[1]};', f'goto {L}_succeed;')
            emit_code(f'{L}_resume:',   f'', f'goto {L}_fail;')
        case 'V':
            L = f'{t[1]}{counter}'
            emit_decl(f'int',           f'{L}_value;')
            emit_code(f'{L}_start:',    f'{L}_value = {t[1]};', f'goto {L}_succeed;')
        ...
        case 'WRITE':
            ...
            emit_decl(f'int',           f'{L}_value;')
            emit_code(f'{L}_start:',    f'', f'goto {E}_start;')
            emit_code(f'{L}_resume:',   f'', f'goto {E}_resume;')
            emit_code(f'{E}_fail:',     f'', f'goto {L}_fail;')
            emit_code(f'{E}_succeed:',  f'{L}_value = write_int(out, {E}_value);', f'goto {L}_succeed;')
        case '+'|'-':
            if len(t) == 2:
                op = t[0]
                ...
                emit_code(f'{L}_resume:',   f'', f'goto {E}_resume;')
                emit_code(f'{E}_fail:',     f'', f'goto {L}_fail;')
                emit_code(f'{E}_succeed:',  f'{L}_value = {op}{E}_value;', f'goto {L}_succeed;')
            elif len(t) == 3:
                op = t[0]
                ...
                E2 = genc(t[2])
                emit_decl(f'int',           f'{L}_value;')
                emit_code(f'{L}_start:',    f'', f'goto {E1}_start;')
                emit_code(f'{L}_resume:',   f'', f'goto {E2}_resume;')
                emit_code(f'{E1}_fail:',    f'', f'goto {L}_fail;')
                emit_code(f'{E1}_succeed:', f'', f'goto {E2}_start;')
                emit_code(f'{E2}_fail:',    f'', f'goto {E1}_resume;')
                emit_code(f'{E2}_succeed:', f'{L}_value = {E1}_value {op} {E2}_value;', f'goto {L}_succeed;')
        case '*'|'/':
            op = t[0]
        ...
        case '<'|'>'|'=='|'<='|'>='|'!=':
            ...
            emit_code(f'{E1}_succeed:', f'', f'goto {E2}_start;')
            emit_code(f'{E2}_fail:',    f'', f'goto {E1}_resume;')
            emit_code(f'{E2}_succeed:', f'if ({E1}_value {nop} {E2}_value)', f'goto {E2}_resume;')
            emit_code(f'',              f'{L}_value = {E2}_value;', f'goto {L}_succeed;')
        case 'TO':
            L = f'to{counter}'
            E1 = genc(t[1])
            E2 = genc(t[2])
            ...
            emit_decl(f'int',           f'{L}_value;')
            emit_code(f'{L}_start:',    f'', f'goto {E1}_start;')
            emit_code(f'{L}_resume:',   f'{L}_i = {L}_i + 1;', f'goto {L}_code;')
            emit_code(f'{E1}_fail:',    f'', f'goto {L}_fail;')
            emit_code(f'{E1}_succeed:', f'', f'goto {E2}_start;')
            emit_code(f'{E2}_fail:',    f'', f'goto {E1}_resume;')
            emit_code(f'{E2}_succeed:', f'{L}_i = {E1}_value;', f'goto {L}_code;')
            emit_code(f'{L}_code:',     f'if ({L}_i > {E2}_value)', f'goto {E2}_resume;')
            emit_code(f'',              f'{L}_value = {L}_i;', f'goto {L}_succeed;')
        case '|':
            op = t[0]
            L = f'alt{counter}'
            E1 = genc(t[1])
            ...
            emit_decl(f'int',           f'{L}_i;')
            emit_decl(f'int',           f'{L}_value;')
            emit_code(f'{L}_start:',    f'{L}_i = 1;', f'goto {E1}_start;')
            emit_code(f'{L}_resume:',   f'if ({L}_i == 1)', f'goto {E1}_resume;')
            emit_code(f'',              f'if ({L}_i == 2)', f'goto {E2}_resume;')
            emit_code(f'{E1}_fail:',    f'{L}_i = 2;', f'goto {E2}_start;')
            emit_code(f'{E1}_succeed:', f'{L}_value = {E1}_value;', f'goto {L}_succeed;')
            emit_code(f'{E2}_fail:',    f'', f'goto {L}_fail;')
            emit_code(f'{E2}_succeed:', f'{L}_value = {E2}_value;', f'goto {L}_succeed;')
        case 'IF':
            L = f'ifstmt{counter}'
            E1 = genc(t[1])
            E2 = genc(t[2])
            ...
            emit_code(f'{E1}_fail:',    f'{L}_gate = addrOf({E3}_resume);', f'goto {E3}_start;')
            emit_code(f'{E1}_succeed:', f'{L}_gate = addrOf({E2}_resume);', f'goto {E2}_start;')
            emit_code(f'{E2}_fail:',    f'goto {L}_fail;')
            emit_code(f'{E2}_succeed:', f'{L}_value = {E2}_value;', f'goto {L}_succeed;')
            emit_code(f'{E3}_fail:',    f'goto {L}_fail;')
            emit_code(f'{E3}_succeed:', f'{L}_value = {E3}_value;', f'goto {L}_succeed;')
    print()
    return L
#------------------------------------------------------------------------------
import timeit
